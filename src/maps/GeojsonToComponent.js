  ...
  switch (type) {
    case `Polygon`:
      return {
        ...
        paths: coordinates.map(geometryToComponentWithLatLng, {
          type: `LineString`
        })[0]
      };
    case `LineString`:
      coordinates = coordinates.map(geometryToComponentWithLatLng, {
        type: `Point`
      });
      return typeFromThis
        ? coordinates
...
export default class GeojsonToComponents extends Component {
  ...
  handleMarkerDragend({ latLng }) {
    ...
    const originalCoordinates =
      marker.properties.originalCoordinates || marker.geometry.coordinates;
      ...
      >
        {features.reduce(
          (array, feature, index) => {
            ...
            const { visible, child, ...featureState } =
              geoStateBy[feature.id] || {};
            if (visible !== false) {
              array.push(
                <ElementClass
                  key={`json-${feature.id}`}
